...
export default function DecisionTables() {
  const { data: rules, isLoading } = useQuery({
    queryKey: ['rules', 'decision'],
    queryFn: () => getRules('decision'),
  });
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Decision Tables</h2>
          <p className="text-gray-600 mt-1">
            Multi-condition decision logic without code
          </p>
        </div>
        <Link
          to="/decision-tables/new"
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex items-center text-sm text-gray-500">
                      ...
                      <span className="truncate">
                        ...
                        {rule.data?.rows?.length || 0} rows
                      </span>
                    </div>
                  </div>
                  <div className="ml-4 flex space-x-2">
                    <Link
                      to={`/decision-tables/${rule.id}`}
      ...
      {!rules || rules.length === 0 ? (
        <div className="text-center py-12 bg-white shadow rounded-lg">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Get started by creating a new decision table.
          </p>
          <div className="mt-6">
            <Link
              to="/decision-tables/new"
